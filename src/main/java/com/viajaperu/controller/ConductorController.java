	...
	public ResponseEntity<?> registrar(@RequestBody Conductor objConductor) {
		...
		try {
			...
			if (CollectionUtils.isEmpty(condlicencia)) {
				...
				salida.put("mensaje", "El Conductor con licencia -> ..." + objConductor.getNrolicencia() + " ...ya se encuentra registrado");
			}
	...
	public ResponseEntity<?>actualizarCondutor(@RequestBody Conductor objConductor){
		...
		try {		
			...
			if(verificar.isPresent()) {				
				...
				List<Conductor> conductorlicencia = serviceconductor.listarConductorXLicenciaDiferenteCodigo(objConductor.getNrolicencia(), objConductor.getCod_conductor());
				if(CollectionUtils.isEmpty(conductorlicencia)) {	
					...
					if(conductorActualizado == null) {
						...
						salida.put("mensaje", "Conductor -> "+ objConductor.getCod_conductor()+ " Actulizado correctamente");
					}
